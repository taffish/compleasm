...
Notes:
  ...
  - The current image is linux/amd64 only. On arm64 machines such as Apple
    Silicon Macs, use Docker's amd64 emulation path, for example:
      DOCKER_DEFAULT_PLATFORM=linux/amd64 TAFFISH_CONTAINER_BACKEND=docker taf-compleasm compleasm --version
    This is not a native arm64 build; Podman or Apptainer on arm64 may need
    separate amd64 compatibility support.
